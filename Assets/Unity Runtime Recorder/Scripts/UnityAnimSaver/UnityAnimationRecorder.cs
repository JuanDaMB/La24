...
namespace Unity_Runtime_Recorder.Scripts.UnityAnimSaver
{
	public class UnityAnimationRecorder : MonoBehaviour {
		...
		public void StartRecordingKey()
		{
			startKey = true;
		}

		public void EndRecordingKey()
		{
			endKey = true;
		}

		private bool startKey = false;
		private bool endKey = false;
	
		// Update is called once per frame
		void Update () {
	
			if (Input.GetKeyDown (startRecordKey) || startKey)
			{
				startKey = false;
				StartRecording ();
			}

			if (Input.GetKeyDown (stopRecordKey) || endKey)
			{
				endKey = false;
				StopRecording ();
			}
