...
namespace Colorful.Editors
{
	...
	public class ShadowsMidtonesHighlightsEditor : BaseEffectEditor
	{
		...
		private SerializedProperty p_Shadows;
		private SerializedProperty p_Midtones;
		private SerializedProperty p_Highlights;
		private SerializedProperty p_Amount;

		private ColorWheel m_ShadowsWheel;
		private ColorWheel m_MidtonesWheel;
		private ColorWheel m_HighlightsWheel;
		...
		{
			// Constants
			private const int kMinSize = 60;

			private const int kMaxSize = 150;

			// Hue Wheel
			private Texture2D m_WheelTexture;
			...
			// UI
			private Texture2D m_ThumbTexture;

			// Input utils
			private Vector2 m_Cursor;

			private ColorWheel m_Dragging;
			...
			private Vector3 GetInput(Rect bounds, Vector3 hsv)
			{
			...
			private void GetWheelHueSaturation(float x, float y, ref float hue, ref float saturation)
			{
			...
			private void UpdateHueWheel(bool sizeChanged)
			{
				if (sizeChanged)
				{
					CleanTexture(m_WheelTexture);
			...
			private void CleanTexture(Texture2D texture)
			{
				if (texture != null)
